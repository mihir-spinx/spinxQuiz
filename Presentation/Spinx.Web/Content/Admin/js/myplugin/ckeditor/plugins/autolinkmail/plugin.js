...
( function() {
	...
	var validUrlRegex = /^(([^<>()\[\]\.,;:\s@\"]+(\.[^<>()\[\]\.,;:\s@\"]+)*)|(\".+\"))@(([^<>()[\]\.,;:\s@\"]+\.)+[^<>()[\]\.,;:\s@\"]{2,})$/ig,
		doubleQuoteRegex = /"/g;

	CKEDITOR.plugins.add( 'autolinkmail', {
		requires: 'clipboard',
		...
		init: function( editor ) {
			editor.on( 'paste', function( evt ) {
				...
				data = data.replace( validUrlRegex , '<a href="mailto:' + data.replace( doubleQuoteRegex, '%22' ) + '" target="_blank">$&</a>' );
